...
/**
 * @brief Process controller for the Star power-up.
...
public:
    /**
     * @brief Constructs a new StarController and initializes the base speed.
     */
    StarController() : Process(), is_deployed(false), is_game_over(false), needs_boost(false), base_vx(-80) {}
    ...
    void init() {
        ...
        watch("deploy_star", [this](Event& e) {
            if (!is_game_over) {
                is_deployed = true;
                teleport(450, 110, 0);
                needs_boost = true;
        ...
        watch("vanish_star", [this](Event& e) {
        ...
        watch("speed_up", [this](Event& e) {
            base_vx -= 20;
            std::cout << "[Star] ID " << id() << " speeding up to " << base_vx << "!" << std::endl;
        });
    }
...
/**
 * @brief The Star agent representing a collectible score multiplier.
 */
class Star : public Agent {
public:
    Star(json spec, World& world) : Agent(spec, world) { add_process(c); prevent_rotation(); }
private:
    StarController c;
};
